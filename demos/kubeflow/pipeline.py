...
preprocess_op = kfp.components.create_component_from_func(
    ...
    output_component_file=os.path.join(
        PROJECT_ROOT, OUTPUT_DIRECTORY, "preprocess-component.yaml"
    ),
...
training_op = kfp.components.create_component_from_func(
    func=train,
    output_component_file=os.path.join(
        PROJECT_ROOT, OUTPUT_DIRECTORY, "train-component.yaml"
    ),
    # This is optional. It saves the component spec for future use.
    base_image="python:3.9",
...
if __name__ == "__main__":
    pipeline_output = os.path.join(PROJECT_ROOT, OUTPUT_DIRECTORY, "wine-pipeline.yaml")
